            ...
            <v-col class="d-flex justify-start flex-wrap align-start" cols="10">
                <v-col class="d-flex justify-start align-start pa-0 ma-0 mb-6" cols="12">
                    <v-text-field
                        ...
                        outlined
                        :error="errorsList && Boolean(errorsList.title)"
                        :error-messages="Boolean(errorsList.title) ? errorsList.title[0] : null"
                    />
                </v-col>
                <v-col class="d-flex justify-start align-start pa-0 ma-0 mb-6" cols="12">
                    <v-text-field
                        ...
                        outlined
                        :error="errorsList && Boolean(errorsList.code)"
                        :error-messages="Boolean(errorsList.code) ? errorsList.code[0] : null"
                    />
                </v-col>
                <v-col class="d-flex justify-start align-start pa-0 ma-0 mb-6" cols="12">
                    <v-autocomplete
...
export default {
    ...
    methods: {
        trySave(exit) {
            ...
            axios.post(`${window.location.pathname}?${queryParams}`, this.section)
                .then(({ data }) => {
                    if (!this.section.id) {
                        this.$inertia.get(data.to)
                    }
                    this.$inertia.reload()
                })
                .catch((response) => {
                    this.errorsList = response.response.data.errors
                });
        },
        uploadPhoto(e) {
