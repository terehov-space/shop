    ...
    <v-col class="d-flex justify-center align-start flex-wrap pa-0 ma-0 middle-margin" cols="12">
        ...
        <v-col class="d-flex justify-center align-start flex-wrap pa-0 ma-0" cols="12" md="7">
            <v-col class="d-flex justify-space-between align-center pa-0 ma-0" cols="12" md="12">
                <v-col class="d-flex justify-start align-center no-gutters px-0" :cols="product.vendorImage ? 8 : 12">
                    <h1 class="text-h5">
                        {{ product.title }}
                    </h1>
                </v-col>
                <v-col class="d-flex justify-end align-center no-gutters" cols="4" v-if="product.vendorImage">
                    <v-img
            ...
            <v-col class="d-flex justify-space-between align-start flex-wrap pa-0 ma-0 info_gutters" cols="12" md="12">
                <v-col class="d-flex justify-start align-start flex-wrap pa-0 ma-0" cols="12" md="6">
                    ...
                    <v-col class="d-flex justify-start align-stretch pa-0 ma-0 char_margin ch-info" cols="12" md="12"
                           v-if="product.vendorCode">
                        <v-col class="d-flex justify-start align-start pa-0 ma-0" cols="6" md="6">
                            Артикул
                        </v-col>
                        <v-col class="d-flex justify-end align-start pa-0 ma-0 text-end" style="text-align: end"
                               cols="6" md="6">
                            {{ product.vendorCode }}
                        </v-col>
                    </v-col>
                    <v-col class="d-flex justify-start align-stretch pa-0 ma-0 char_margin ch-info" cols="12" md="12"
                           v-if="product.vendorTitle">
                        <v-col class="d-flex justify-start align-start pa-0 ma-0" cols="6" md="6">
                            Производитель
                        </v-col>
                        <v-col class="d-flex justify-end align-start pa-0 ma-0 text-end" style="text-align: end"
                    ...
                    <template v-for="(ch, i) in product.properties">
                        <v-col class="d-flex justify-start align-stretch pa-0 ma-0 char_margin ch-info" cols="12"
                               md="12"
                               :key="i">
                            <v-col class="d-flex justify-start align-start pa-0 ma-0" cols="6" md="6">
                                {{ ch.title }}
                    ...
                    <template v-if="product.files && product.files.length > 0">
                        <v-col class="d-flex justify-start align-start flex-column pa-0 ma-0 char_margin ch-info"
                               cols="12" md="12">
                            <template v-for="file in product.files">
                                <a :href="file.path" style="margin: 8px 0 8px 0" target="_blank"
                                   :key="file.id">{{ file.title ? file.title : file.filename }}</a>
                            </template>
                        </v-col>
                    </template>
    ...
    computed: {
        hasDescription() {
          if (this.product.description) {
              return Boolean(this.product.description.replace(/<[^>]*>?/gm, ''))
          }
    ...
    head() {
        return {
            ...
            meta: [
                {
                    name: 'description',
                    content: this.product.seo_description ? this.product.seo_description : this.seoDescription,
                },
                {
                    name: 'og:title',
                    content: this.product.seo_title ? this.product.seo_title : `${this.product.title} - Заказывайте у нас с доставкой по всей России`,
                },
                {
                    name: 'og:description',
                    content: this.product.seo_description ? this.product.seo_description : this.seoDescription,
